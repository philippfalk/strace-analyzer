            ...
            for lineno, line in enumerate(fh):
                ...
                if "execve" in line:
                    ...
                    file_access_stats[filename]["open_modes"].append(
                        file_access_stats[filename]["open_modes"][-1]
                    )
                    ...
                    file_access_stats[filename]["open_from"][inputfile] = (
                        file_access_stats[filename]["open_from"][inputfile] + 1
                    )
                elif "open(" in line:
                    # logging.debug("OPEN:")
                    match = re.search(
                    ...
                    if dirfd != "AT_FDCWD" and not os.path.isabs(filename):
                        filename = (
                            open_file_tracker.get_filename(int(dirfd)) + "/" + filename
                        )
                    open_file_tracker.register_open(filename, fd)
                    if filename not in file_access_stats:
                        file_access_stats[filename] = new_file_access_stats_entry(
                    ...
                    if not open_file_tracker.is_open(fd):
                        logging.warning(
                            "Closing unrecognized file descriptor {0}".format(fd)
                        )
                        continue
                    filename = open_file_tracker.get_filename(fd)
                    file_access_stats[filename]["close_times"].append(
                        float(match.group("close_time"))
                    )
                    open_file_tracker.register_close(fd)
                elif (
                    "write(" in line or "writev(" in line or "write64(" in line
                ) and not "process_vm_writev" in line:
                    ...
                    if open_file_tracker.is_open(fd):
                        filename = open_file_tracker.get_filename(fd)
                        file_access_stats[filename]["write_times"].append(
                            float(match.group("write_time"))
                        )
                        file_access_stats[filename]["write_sizes"].append(
                            int(match.group("write_size"))
                        )
                    else:
                        logging.warning(
                ...
                ) and not "process_vm_readv" in line:
                    ...
                    if open_file_tracker.is_open(fd):
                        filename = open_file_tracker.get_filename(fd)
                        file_access_stats[filename]["read_times"].append(
                            float(match.group("read_time"))
                        )
                        file_access_stats[filename]["read_sizes"].append(
                            int(match.group("read_size"))
                        )
                    else:
                        logging.warning(
                    ...
                    match = re.search(
                        r"(?P<difftime>[0-9]+\.[0-9]+) (?P<func>.*?)\(.*\).*=.*<(?P<time>[0-9]+\.[0-9]+)>",
                        line,
                    )
                    if match != None:
                        # logging.debug('{0}'.format(match.groupdict()))
                        ...
                        unknown_calls[callname]["count"] = (
                            unknown_calls[callname]["count"] + 1
                        )
...
def calc_file_access_stats(file_access_stats):
    for filename in file_access_stats.keys():
        file_access_stats[filename]["write_time"] = sum(
            file_access_stats[filename]["write_times"], 0.0
        )
        file_access_stats[filename]["write_count"] = len(
            file_access_stats[filename]["write_times"]
        )
        file_access_stats[filename]["write_size"] = sum(
            file_access_stats[filename]["write_sizes"]
        )
        file_access_stats[filename]["read_time"] = sum(
            file_access_stats[filename]["read_times"], 0.0
        )
        file_access_stats[filename]["read_count"] = len(
            file_access_stats[filename]["read_times"]
        )
        file_access_stats[filename]["read_size"] = sum(
            file_access_stats[filename]["read_sizes"]
        )
        file_access_stats[filename]["open_time"] = sum(
            file_access_stats[filename]["open_times"], 0.0
        )
        file_access_stats[filename]["open_count"] = len(
            file_access_stats[filename]["open_times"]
        )
        file_access_stats[filename]["close_time"] = sum(
            file_access_stats[filename]["close_times"], 0.0
        )
...
def main():
    optparser = optparse.OptionParser(
        "usage: %prog [options] STRACE_LOG ...", version="%prog 0.1"
    )
    ...
    if options.sort_by not in all_properties:
        logging.error(
            "Unknown sort-by value '{0}'. Falling back to '{1}'".format(
    ...
    sorted_filenames = sorted(
        file_access_stats.values(), reverse=True, key=lambda k: k[sort_by]
    )
    ...
    if options.unknown_call_stats:
        ...
        for callname, time in sorted(unknown_call_times.items()):
            print(
                "{0:16} {1:>12} {2:>12.9}".format(
                    callname, unknown_calls[callname]["count"], time
                )
            )
